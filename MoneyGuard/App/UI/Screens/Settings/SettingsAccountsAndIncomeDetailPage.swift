...
struct SettingsAccountsAndIncomeDetailPage: View {
    ...
    @State private var isOpeningBalanceRevealed = false
    @FocusState private var isOpeningBalanceFocused: Bool

    private let paymentMethodTypeOptions = ["cash", "bank", "wallet", "credit"]

    private var shouldMaskOpeningBalance: Bool {
        hideBalances && !isOpeningBalanceRevealed
    }

    private var openingBalanceDisplayText: String {
        shouldMaskOpeningBalance ? "••••••" : AppCurrency.formatted(openingBalance)
    }
            ...
            VStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    HStack(spacing: 12) {
                        ...
                        Spacer()
                        Text(openingBalanceDisplayText)
                            .font(.system(.body, design: .rounded).weight(.semibold))
                            .foregroundStyle(palette.accent)
                            .onTapGesture {
                                if shouldMaskOpeningBalance {
                                    isOpeningBalanceRevealed = true
                                }
                            }
                    }
                    ...
                    if shouldMaskOpeningBalance {
                        Button {
                            ...
                            Text(String(localized: "Tap to reveal balance"))
                                .font(.caption)
                                .foregroundStyle(palette.secondaryInk)
                        }
                        .buttonStyle(.plain)
