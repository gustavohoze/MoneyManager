        ...
        NavigationStack {
                ...
                .overlay(alignment: .bottom) {
                    if let toast {
                        UniversalToastView(
                            state: toast,
                            palette: palette,
                            onUndo: viewModel.canUndoLastSave ? {
                                focusedField = nil
            ...
            .onChange(of: viewModel.toastEventID) { _, _ in
                if let error = viewModel.error {
                    toast = UniversalToastState(
                        message: viewModel.errorMessage(for: error),
                        isError: true
                    )
                } else if let saveMessage = viewModel.saveMessage, !saveMessage.isEmpty {
                    toast = UniversalToastState(
                        message: saveMessage,
                        isError: false,
                        undoTitle: viewModel.canUndoLastSave ? String(localized: "Undo") : nil
                    )
                } else {
                    return
                }
