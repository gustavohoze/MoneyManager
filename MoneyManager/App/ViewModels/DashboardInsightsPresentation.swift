...
extension DashboardViewModel {
    ...
    var derivedAlerts: [DashboardAlert] {
        ...
        if isWeeklyBudgetUserConfigured,
           weeklyBudget > 0,
           weeklySpending > 0 {
            let usedPercent = Int((weeklyProgress * 100).rounded())
            merged.insert(
                DashboardAlert(
                    title: "⚠︎ " + String(localized: "Budget usage"),
                    detail: "\(usedPercent)% " + String(localized: "of your weekly budget is used.")
                ),
                at: 0
            )
        }
